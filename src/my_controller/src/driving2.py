...
class Controller():
	...
	def __init__(self):
		...
		points_arr = np.loadtxt('path_30_2.txt')
	...
	def _take_action_distribute(self):
		lin_vel_add = self.pi[self.index][1] * 0.4 +  self.pi[self.index][2] * 0.4 +  self.pi[self.index][3] * 0.4 - \
		...
		self.ang_vel = 4 * self.pi[self.index][1] + 4* self.pi[self.index][4] + 4* self.pi[self.index][6] - \
				4.5 * self.pi[self.index][3] - 4.5 * self.pi[self.index][5] - 4.5 * self.pi[self.index][8]
	...
	def _process_image(self, msg):
		...
		elif self._check_offroad() or self.still > 10:
			self._respawn()
			#self.sign_pub.publish(str('nootnoot,multi21,0,NA'))
			self.still = 0
			self.d[self.index-1] = 1
			self.r[self.index-1] = 0.0
		if (self.curr_idx == len(self.points)-1) or (self.pos[0] < -4.0):
	...
	def _record_points(self):
		...
		with open('path_30_2.txt', 'a') as file:
	...
	def _respawn(self):
		...
		respawn_num = np.random.rand()
		if respawn_num < 0.5:
			random_num = np.random.rand()
			#down
			msg.pose.position.x = 0.5136 + 0.2 * (np.random.random() - 0.5)
			msg.pose.position.y = -0.572393 + 0.2 * (np.random.random() - 0.5)
			msg.pose.position.z = 0.0525
			msg.pose.orientation.y = 0
			msg.pose.orientation.x = 0
			msg.pose.orientation.z = 0.707 + 0.4 * (np.random.random() - 0.5)
			msg.pose.orientation.w = np.sqrt(1 - msg.pose.orientation.z**2)
			self.curr_idx = 0
		else:
			#right
			msg.pose.position.x = 1.8136
			msg.pose.position.y = 2.372393
			msg.pose.position.z = 0.0525
			msg.pose.orientation.y = 0
			msg.pose.orientation.x = 0
	...
	def run(self):
		...
		msg.pose.position.x = 0.543696
		msg.pose.position.y = -0.572393
		msg.pose.position.z = 0.0525
		msg.pose.orientation.y = 0
		msg.pose.orientation.x = 0
