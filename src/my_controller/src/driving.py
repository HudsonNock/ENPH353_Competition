...
class myModel(nn.Module):
	...
	def __init__(self, i, channels):
		...
		self.dense1 = nn.Linear(22722, 256)
...
class Controller():
	...
	def __init__(self):
		...
		points_arr = np.loadtxt('path_30.txt')
		...
		# so |A| = 18
		self.H = 1.01 #0.98 * (- np.log(1 / 3.0))

		self.channels = 1
		self.input_shape = (self.channels, 360, 640)
		# pi, Q1, Q2, Qt1, Qt2
		self.models = nn.ModuleList()
		self.weights_folder = ['weights2/pi.h', 'weights2/Q1.h5', 'weights2/Q2.h5', 'weights2/Qt1.h5', 'weights2/Qt2.h5']
		self._initalize_models()

		self.alpha = nn.Parameter(torch.tensor(0.01), requires_grad=False)
		self.batch_size = 1
		self.gamma = 0.99
		self.tau = 0.003
		#Replay buffer
		...
		self.pi = np.zeros(shape=(self.batch_size+1, 3), dtype=np.float32)
		...
		optimizer_phi = optim.Adagrad(self.models[0].parameters(), lr=0.001)
		optimizer_q1 = optim.Adagrad(self.models[1].parameters(), lr=0.001)
		optimizer_q2 = optim.Adagrad(self.models[2].parameters(), lr=0.001)
		optimizer_alpha = optim.Adagrad([self.alpha], lr=0.0005)
	...
	def _find_closest_point(self):
		...
		passed = False
		for i in range(self.curr_idx + 1, min(self.curr_idx + 11, len(self.points))):
	...
	def _process_image(self, msg):
		...
		#cv2.waitKey(1)
		self.d[self.index-1] = self._check_terminal_state()

		self.s_vel[self.index] = np.array([self.lin_vel / 10.0, self.ang_vel / 20.0])
	...
	def _train(self):
		...
		#one_hot = torch.tensor(np.eye(3)[self.a[0:self.index]], requires_grad=False).cuda()
		#selected_q_values = torch.sum(q[0:self.index] * one_hot, dim=-1)
		#with torch.no_grad():
		#	delta = self._critic_loss(q) #selected - target
		#loss = torch.mean(selected_q_values * delta)
		...
		#if self.alpha < 0:
		#	self.alpha = nn.Parameter(torch.tensor(0.005), requires_grad=False)
	...
	def _critic_loss(self, q):
		...
		#one_hot = torch.tensor(np.eye(3)[self.a[0:self.index]], requires_grad=False).to(q.dtype).cuda()
		...
		#selected_q_values = torch.sum(q[0:self.index] * one_hot, dim=-1)
		loss = 0.5 * torch.mean((q[0:self.index] - target_q_values)**2)
		#print("A")
		#print(q[0:self.index])
		#print(r)
		#print(d)
		#print(q[1:self.index+1])
