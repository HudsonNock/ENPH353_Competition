...
class Controller():
	...
	def __init__(self):
		...
		points_arr = np.loadtxt('path_30_3.txt')
		...
		# so |A| = 18
		self.H = 0.6 #0.8 #0.98 * (- np.log(1 / 3.0))
		...
		self.weights_folder_best = ['weights_1.4Speed_3/pi.h5', 'weights_1.4Speed_3/Q1.h5', 'weights_1.4Speed_3/Q2.h5', 'weights_1.4Speed_3/Qt1.h5', 'weights_1.4Speed_3/Qt2.h5']

		self._initalize_models()
		self.alpha = nn.Parameter(torch.tensor(0.0047), requires_grad=False)
		self.log_alpha = nn.Parameter(torch.tensor(-5.5))
		self.batch_size = 1
		self.gamma = 0.97
		self.tau = 0.002
		#Replay buffer
	...
	def _take_action_distribute(self):
		...
		self.ang_vel = 2.0*(4 * self.pi[self.index][1] + 4* self.pi[self.index][4] + 4* self.pi[self.index][6] - \
	...
	def _process_image(self, msg):
		...
		elif self._check_offroad() or self.still > 100: #8:
			self._respawn()
			#self.sign_pub.publish(str('nootnoot,multi21,0,NA'))
			self.still = 0
			self.d[self.index-1] = 1
			self.r[self.index-1] = 0.0
		if (self.curr_idx == len(self.points)-1) or (self.pos[0] > -2.5 and self.pos[1] < -0.86):
	...
	def _record_points(self):
		...
		with open('path_30_3.txt', 'a') as file:
	...
	def _respawn(self):
		...
		if random_num < 0.25:
			msg.pose.position.x = -3.043696
			msg.pose.position.y = 1.302393
			msg.pose.position.z = 0.0525
			msg.pose.orientation.y = 0
			msg.pose.orientation.x = 0
			msg.pose.orientation.z = 0.93 + 0.04 * (np.random.random() - 0.5)
			msg.pose.orientation.w = -np.sqrt(1 - msg.pose.orientation.z**2)
			self.Rcnt = 6
		else:
			msg.pose.position.x = -4.043696 - 0.5 * np.random.random()
			msg.pose.position.y = -1.302393
			msg.pose.position.z = 0.0525
			msg.pose.orientation.y = 0
			msg.pose.orientation.x = 0
	...
	def run(self):
		...
		msg.pose.position.x = -3.243696
		msg.pose.position.y = 1.302393
		msg.pose.position.z = 0.0525
		msg.pose.orientation.y = 0
		msg.pose.orientation.x = 0
		msg.pose.orientation.z = 0.93
		msg.pose.orientation.w = -np.sqrt(1 - msg.pose.orientation.z**2)
