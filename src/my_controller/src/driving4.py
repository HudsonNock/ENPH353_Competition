...
class Controller():
	...
	def __init__(self):
		...
		points_arr = np.loadtxt('path_30_4.txt')
		...
		self.weights_folder_best = ['weights_2.5Speed_2/pi.h5', 'weights_2.5Speed_2/Q1.h5', 'weights_2.5Speed_2/Q2.h5', 'weights_2.5Speed_2/Qt1.h5', 'weights_2.5Speed_2/Qt2.h5']

		self._initalize_models()
		self.alpha = nn.Parameter(torch.tensor(0.0047), requires_grad=False)
		self.log_alpha = nn.Parameter(torch.tensor(-5.05))
		self.batch_size = 1
		self.gamma = 0.97
		self.tau = 0.002
		#Replay buffer
	...
	def _find_closest_point(self, length=21):
		maxI = self.curr_idx
		d_curr = np.sum((self.points[self.curr_idx] - self.pos)**2)
		passed = False
		for i in range(self.curr_idx + 1, min(self.curr_idx + length, len(self.points))):
	...
	def _take_action_distribute(self):
		...
		self.ang_vel = 1.1*(4 * self.pi[self.index][1] + 4* self.pi[self.index][4] + 4* self.pi[self.index][6] - \
	...
	def _process_image(self, msg):
		...
		if self.hasR:
			...
			self._check_offroad()
			self.curr_idx = 0
			self._find_closest_point(length=1000)
			self.rwd = 0
			self.hasR = False
		if (self.curr_idx == len(self.points)-1) and self.lin_vel == 0:
		...
		if self.hit:
			...
			self.still = 0
			self.d[self.index-1] = 1
			self.r[self.index-1] = 0
		elif self._check_offroad() or (self.still > 12 and self.curr_idx != len(self.points)-1): #8:
			self._respawn()
			#self.sign_pub.publish(str('nootnoot,multi21,0,NA'))
			self.still = 0
			self.d[self.index-1] = 1
		...
		if self.hasR == False:
			...
			if self.index != 0 and self.curr_idx != len(self.points)-1:
			...
			if self.curr_idx == len(self.points)-1:
				if self.a[self.index-1] == 7:
					self.r[self.index-1] = 0.02
				else:
					self.r[self.index-1] = 0.0
	...
	def _record_points(self):
		...
		with open('path_30_4.txt', 'a') as file:
	...
	def _respawn(self):
		...
		if random_num < 1.0: #0.34:
			msg.pose.position.x = -2.2215
			msg.pose.position.y = -2.3115 #+ 0.2 * (np.random.random() - 0.5)
			msg.pose.position.z = 0.656542
			msg.pose.orientation.y = 0.17
			msg.pose.orientation.x = 0
			msg.pose.orientation.z = 0 #0.05* np.random.random()
			#if np.random.random() > 1.0:
			msg.pose.orientation.w = -np.sqrt(1 - msg.pose.orientation.y**2 - msg.pose.orientation.z**2)
			#else:
			#	msg.pose.orientation.w = np.sqrt(1 - msg.pose.orientation.y**2 - msg.pose.orientation.z**2)
			self.Rcnt = 10
		elif random_num < 0.0:
			msg.pose.position.x = -0.4734 + 0.1 * np.random.random()
			msg.pose.position.y = -0.632470
			msg.pose.position.z = 1.257744
			msg.pose.orientation.y = 0.17
			msg.pose.orientation.x = 0
			msg.pose.orientation.z = 0.707 + 0.1* np.random.random()
			msg.pose.orientation.w = np.sqrt(1 - msg.pose.orientation.y**2 - msg.pose.orientation.z**2)
			self.Rcnt = 10
		else:
			msg.pose.position.x = -2.505231 + 0.1 * np.random.random()
			msg.pose.position.y = 0.074561
			msg.pose.position.z = 1.494623
	...
	def run(self):
		...
		msg.pose.position.x = -2.6815
		msg.pose.position.y = -2.3115
		msg.pose.position.z = 0.406542
		msg.pose.orientation.y = 0.17
		msg.pose.orientation.x = 0
